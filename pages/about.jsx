...
const About = () => (
  <AppShell
    title={`Vijay Chandar | ${USER.about.title}`}
  ...
  >
    ...
    {USER.about.contents.map((item) => (
      <AppSection key={item.id} title={item.name}>
        <p className="text-gray-900 dark:text-gray-100 text-xs md:text-base leading-5 md:leading-8 text-center max-w-2xl lg:mx-auto">
          {item.description}
        </p>
      </AppSection>
    ))}
